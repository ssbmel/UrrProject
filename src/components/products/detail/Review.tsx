          ...
          {Array(fullStars)
            ...
            .map((_, index) => (
              <div key={`full-${index}`} className="w-4 h-4 relative">
                <Image src={fullStar} alt="full star" fill sizes="16px" className="rounded-md object-cover" />
              </div>
            ))}
          {hasHalfStar && (
            <div className="w-4 h-4 relative">
              <Image src={halfStar} alt="half star" fill sizes="16px" className="rounded-md object-cover" />
            </div>
          )}
          {Array(emptyStars)
            .fill(null)
            .map((_, index) => (
              <div key={`empty-${index}`} className="w-4 h-4 relative">
                <Image src={emptyStar} alt="empty star" fill sizes="16px" className="rounded-md object-cover" />
              </div>
            ))}
        </div>
      </div>
